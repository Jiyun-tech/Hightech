...
int main () {
	...
	long long inputSocialID; 	// 사용자 주민등록번호 
	...
	do { // ~ while => return to beginning of the process if the user select "continue purchasing ticket" at the end of the process. 
		...
		do {
			...
			scanf("%lld", &inputSocialID);
		} while (inputSocialID < 99999 || inputSocialID > 9999999 || inputSocialID%100000/1000 > 12 || inputSocialID%1000/10 > 31 || inputSocialID%10/1 > 4);
		// 주민등록번호 오류 시 입력창 재출력. 
		...
		// International Age of the user.
		birthYearIndex = inputSocialID%10/1; 
		if (birthYearIndex == INDEX_MALE_OLD || birthYearIndex == INDEX_FEMALE_OLD) {
			userBirthYear = (inputSocialID/100000) + BIRTH_YEAR_ADD_OLD;
		} else if (birthYearIndex == INDEX_MALE_NEW || birthYearIndex == INDEX_FEMALE_NEW) {
			userBirthYear = (inputSocialID/100000) + BIRTH_YEAR_ADD_NEW;
		} 
		userBirthMonth = inputSocialID%100000/1000;
		userBirthDay = inputSocialID%1000/10;
