import java.text.DecimalFormat;		// 숫자 출력 형식에 콤마를 추가하기 위한 패키지
import java.text.SimpleDateFormat;	// 현재 날짜,시간 출력 양식 지정 위한 패키지
import java.util.ArrayList;			// ArrayList -> 상품별 면세 여부 표시 및 출력용 상품명 저장용.
import java.util.Calendar;			// 현재 날짜,시간 받아오기 위한 패키지
//KOPO18 서지윤 / 2022.04.26~28
...
public class K18_receipt03 {
	...
	public static void main(String[] args) {
		...
		// 상품명
		String[] k18_itemname = {"맛있는 군밤 100g x 4개입","해남 고구마 말랭이","찌개용두부(500g)","송산 포도","나주 배",
								...
								"체리부로닭볶음용800g","새송이(봉)","이맛쌀20kg","50년근 산삼","후디스 그릭요거볼 플레인"};
		...
		// 상품 가격 (개당 단가)
		int[] k18_itemprice = {7000,2000,3000,1200,500,3900,3900,3950,20800,3980,
								25000,15000,8000,3000,4000,2960,2980,4400,4580,6580,
								6990,12000,4000,2000,5000,5580,3280,51990,9999990,4480};
		// 상품별 구매 수량
		int[] k18_itemnumber = {2,3,1,5,5,1,2,10,1,2,
								1,4,7,1,1,3,1,1,2,2,
								3,1,1,1,2,2,2,1,1,6};
		// 상품별 면세/과세 여부 (true = 면세, false = 과세)
		boolean[] k18_taxfree = {false,false,true,true,true,true,true,true,true,false,
								true,true,true,true,true,false,false,false,false,false,
								false,false,false,false,false,true,true,true,true,false};
		
		
		// #2. 상품 총 소비자 가격, 부가세, 과세 전 가격 계산
		...
		// #2-2. 면세 상품 총액 & 과세 상품 (세후/세전) 총액 계산
		...
		// #2-3. 부가세 가격 계산 = 소비자가격 총액 - 면세상품 가격 총액 - 과세상품 세전가격 총액;
		...
		// #3. 출력 상품명 글자수 지정하기 => 출력부 하단에 메서드(k18_printName) 생성하여 사용.
		// 메서드 호출 및 인수 지정하여 출력용 상품명 ArrayList에 값 저장 
		// (필요 인수 : 상품명 배열, 길이(바이트), 상품명 저장 ArrayList)
		K18_receipt03.k18_printName(k18_itemname, k18_printLength, k18_itemnamePrint);

		// #4. 숫자 세 개 단위로 콤마 찍기 위해 DecimalFormat 클래스 객체화 및 형식 지정
		DecimalFormat k18_comma = new DecimalFormat("###,###,###");
		
		// #5. 계산 일시 & 차량 입차 일시 찍기 -> Calendar & SimpleDateFormat 사용
		// (1) Calendar형 객체 선언 및 인스턴스화.
		Calendar k18_cal = Calendar.getInstance(); 	// 구매 일시 출력용
		Calendar k18_cal2 = Calendar.getInstance();	// 차량 입차시간 출력용
		// (2) SimpleDateFormat 형 객체 선언 및 형식 초기화 ("연연연연.월월.일일 시시:분분") // 구매 시간용
		SimpleDateFormat k18_sdf1 = new SimpleDateFormat ("YYYY-MM-dd HH:mm");
		// (3) SimpleDateFormat 형 객체 선언 및 형식 초기화 ("연연연연-월월-일일 시시:분분:초초") // 차량 입차시간용
		SimpleDateFormat k18_sdf2 = new SimpleDateFormat ("YYYY.MM.dd HH:mm:ss");
		// (4) SimpleDateFormat 형 객체 선언 및 형식 초기화 ("연연연연월월일일") // 바코드 하단용
		SimpleDateFormat k18_sdf3 = new SimpleDateFormat ("YYYYMMdd");
		// (5) 차량 입차 시간 계산
		int k18_shoppingMinutes = 57;							// 정수형 변수 선언 및 쇼핑 소요 시간(분)으로 초기화;
		k18_cal2.add(Calendar.MINUTE, -k18_shoppingMinutes);	// 현재 시간 - 쇼핑 소요 시간(minutes) = 차량 입차 시간;

		// #6. 출력 시작
		// 출력 형식 지정 시 %와 s,d 등 형식 사이에 숫자를 넣어 출력 칸 조정 (양수 : 우측 정렬, 음수 : 좌측 정렬)
		System.out.printf("%-14s%-10s\n","emart","이마트 죽전점 (031)888-1234"); // 총 42bytes (최대 출력 길이)
		System.out.printf("%-14s%-10s\n","","206-86-50913 강희석");
		System.out.printf("%-14s%-10s\n","","용인 수지구 포은대로 552");
		System.out.printf("\n");
		System.out.printf("%s\n", "영수증 미지참시 교환/환불 불가");
		...
		for (int k18_i = 0; k18_i < k18_itemname.length; k18_i++) {
		...
		// 차량 입차 시간 출력 - 예제 영수증 기준(결제 시간 57분 전 입차)으로 계산.
		System.out.printf("%-8s%29s\n", "입차시간 : ",k18_sdf2.format(k18_cal2.getTime()));
		System.out.printf("-----------------------------------------\n"); // 특수문자 : 41개
	...
	// #3-2. 출력 상품명 글자수 지정 메서드 : 상품명 출력 Byte에 맞추고, ArrayList에 저장 =>글자수 Byte로 지정, 상품명을 Byte로 변환하여 출력 기준과 비교. 
	// *참고: 한글은 한 글자당 2byte이며 모두 음수값. (영어, 숫자, 특수문자 등은 1byte & 양수)
	public static ArrayList<String> k18_printName (String[] k18_itemName, int k18_byteLenght, ArrayList<String> k18_printName) {
	// 메서드 k18_printName 생성 -> 메서드 실행 시 필요 인수 지정 : 상품명 배열, 길이(바이트), 출력용 상품명 저장 ArrayList
		for (int k18_i = 0; k18_i < k18_itemName.length; k18_i++) { 	// 상품 종류만큼 for문 반복
		...
		return k18_printName; // 메서드 실행 완료 후, k18_printName ArrayList가 완성되어 return됨.
	}
}
