...
public class k18_receipt02 {
	...
	public static void main(String[] args) {
		...
		// #4. 출력 상품명 글자수 지정하기
		ArrayList<String> k18_itemnamePrinteAll = new ArrayList<String>();
		for (int k18_i = 0; k18_i < k18_itemnameAll.size(); k18_i++) { 			// 상품 종류만큼 for문 반복
			...
			int length = 22;													// 출력할 상품명 길이(byte) 지정
			if (k18_byteString.length <= length) {								// 조건 : 상품명 길이 <= 출력 지정 길이
				k18_itemnamePrinteAll.add(k18_itemnameAll.get(k18_i));			// 실행 : 상품명 그대로 출력용 ArrayList에 저장
			} else {															// 조건 : 상품명 길이 > 출력 지정 길이
				int k18_minusByteCount = 0;										// 실행 : 상품명에서 사용할 길이(Byte)계산.
				for (int k18_j=0; k18_j<length; k18_j++) {
				...
				k18_itemnamePrinteAll.add(new String(k18_byteString, 0, length)); 
				// 상품명 지정 byte 길이에 맞추어 잘라서 -> 출력용 ArrayList에 저장
			}
		}
